...
public class VastVideoViewProgressRunnableTwoTest {
    @Mock VastVideoViewControllerTwo mockVastVideoViewController;
    @Mock Context mockContext;
    @Mock Handler mockHandler;
    @Mock MoPubRequestQueue mockRequestQueue;
    @Mock VastVideoConfigTwo mockVideoConfig;
    @Captor ArgumentCaptor<TrackingRequest> requestCaptor;
    VastVideoViewProgressRunnableTwo subject;

    @Before
    public void setup() {
        subject = new VastVideoViewProgressRunnableTwo(mockVastVideoViewController, mockVideoConfig,
                mockHandler);
    ...
    public void doWork_whenTrackersReturned_shouldMakeTrackingRequests() {
        List<VastTrackerTwo> testTrackers = new ArrayList<>();
        testTrackers.add(new VastAbsoluteProgressTrackerTwo.Builder("https://example.com/", 1999).build());
        testTrackers.add(new VastAbsoluteProgressTrackerTwo.Builder("https://example1.com/", 2000).build());
